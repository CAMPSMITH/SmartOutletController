...
from utils import get_config, initialize_devices, get_desired_state,ConfigurationError
import json
...
@api.get("/api/device/{device_name:path}")
...
@api.get("/api/plug/{device_name:path}/{plug:path}")
...
@api.post("/api/device/{device_name:path}")
...
@api.post("/api/event/{device_name:path}",status_code=202)
